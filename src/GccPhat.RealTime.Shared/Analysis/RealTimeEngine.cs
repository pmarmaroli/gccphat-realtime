...
public sealed class RealTimeEngine : IDisposable
{
    ...
    private readonly Func<IAudioRenderDeviceInfo?, IAudioOutputSink> _createOutputSink;
    private Beamformer? _beamformer;
    private IAudioOutputSink? _sink;
    private IAudioRenderDeviceInfo? _renderDevice;
    ...
    public RealTimeEngine(Func<IAudioRenderDeviceInfo?, IAudioOutputSink> createOutputSink)
    {
        _createOutputSink = createOutputSink;
    }
    ...
    private void SynthesizeBeamOutput()
    {
        lock (_audioGate)
        {
            if (!_listening || _beamformer is null || _capture is null || _sink is null)
            {
                return;
            }
    ...
    public void SetRenderDevice(IAudioRenderDeviceInfo? renderDevice)
    {
        lock (_audioGate)
        {
    ...
    private void RebuildOutputLocked(int sampleRate)
    {
        ...
        _sink = _createOutputSink(_renderDevice);
        _sink.Start(sampleRate, 1, blockMs);
    }
