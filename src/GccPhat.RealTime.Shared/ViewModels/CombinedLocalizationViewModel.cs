...
public sealed class CombinedLocalizationViewModel : ObservableObject, IDisposable
{
    ...
    private PairStatus _crossCheckStatus = PairStatus.Weak;
    private readonly IUiTimer _crossCheckTimer;

    public CombinedLocalizationViewModel(IUiDispatcher dispatcher)
    {
        _crossCheckTimer = dispatcher.CreateRepeatingTimer(TimeSpan.FromMilliseconds(300), UpdateCrossCheck);
    ...
    public PairStatus CrossCheckStatus
    {
        get => _crossCheckStatus;
        private set => SetProperty(ref _crossCheckStatus, value);
    }
    ...
    private void UpdateCrossCheck()
    {
        if (_calibration is not SyncCalibration calibration)
        {
            ...
            CrossCheckStatus = PairStatus.Weak;
            return;
        }
        if (_sessionA is null || _sessionB is null)
        {
        ...
        CrossCheckStatus = Math.Abs(diffSamples) <= CrossCheckGoodToleranceSamples ? PairStatus.Good : PairStatus.Weak;
        CrossCheckText =
