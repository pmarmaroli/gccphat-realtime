    ...
    private void Redraw()
    {
        ...
        foreach (PairViewModel pair in _viewModel.ActivePairs)
        {
                ...
                channelScreen.TryGetValue(pair.Pair.ChannelB, out var posB))
            {
                ...
                (byte r, byte g, byte b) = Palette.Get(pair.PaletteIndex);
                ArrayGeometryCanvasDrawing.DrawPairLine(c, posA.X, posA.Y, posB.X, posB.Y, new SolidColorBrush(Color.FromRgb(r, g, b)), isUsed);
            }
        }
